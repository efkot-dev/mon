			...
			foreach($data['port'] as $idp => $valuePon){
				if(preg_match('/GPON/i',$valuePon['name'])) {
					$listPon[$idp] = [
						...
						'cardcount' => (isset($valuePon['support'])?$valuePon['support']:128)
					];
					$sort++;
					...
					$listPon[$idp] = [
						...
						'llid' => $valuePon['id'],
						'typeport' => $valuePon['typeport'],
						'cardcount' => (isset($valuePon['support'])?$valuePon['support']:64)
					];
					$sort++;
				}
			}
