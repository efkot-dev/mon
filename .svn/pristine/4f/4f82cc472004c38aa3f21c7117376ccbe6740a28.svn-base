...
class Vsolv16_Epon{ 
    ...
    private $indexdevice;
    private $now;
    private $primary = 'status,name,inface';
    private $configapionuepon = 'rx,tx,dist,status,mac,inface,vendor,model,volt';
	...
	public function Load(){
		$result = [];
		$epon = [
			'oid' => '1.3.6.1.4.1.37950.1.1.5.12.1.12.1.6','type' => 'class','ip' => $this->ip,'community'=> $this->community
		];
		$indexonuepon = pmon_walk($epon);
		if(is_array_empty($indexonuepon)){
			foreach($indexonuepon as $infacex => $data_epon) {
				$io = trim($infacex);
				if($io && !empty($data_epon['result'])) {
					$mac = getsnmp_string($data_epon['result']);
					if(isset($mac)){
						$result[$io] = [
							'do' => 'onu','id' => $this->id,'mac' => $mac,'pon' => 'epon','types' => $this->primary,'keyonu' => $io
						];
					}
				}
			}
	...
	public function Port(){		
		$gpon = [
			...
			'type' => 'class','deloid' => true,
			'ip' => $this->ip,'community'=> $this->community
		];
		$listPort = [];
		$data = [];
