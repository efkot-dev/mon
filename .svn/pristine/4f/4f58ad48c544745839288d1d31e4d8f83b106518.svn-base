...
$speedbar = '<a class="brmhref" href="/?do=location"><i class="fi fi-rr-building"></i>'.$lang['location'].'</a>';
if($id)
	$datalocation = $db->Fast('location','*',['id'=>$id]);
if(!empty($datalocation['id'])){
	...
	$speedbar .='<span class="brmspan"><i class="fi fi-rr-angle-left"></i>'.$datalocation['name'].'</span></div>';
	...
	$SQLListlocation = $db->Multi('location');
	if(count($SQLListlocation)){
		foreach($SQLListlocation as $location){
			...
			$countSqlDevice = $db->Multi('switch','id',['location'=>$location['id']]); 
