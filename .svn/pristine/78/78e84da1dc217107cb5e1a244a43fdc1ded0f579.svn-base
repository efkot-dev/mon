...
$urlswitch ='';
$tplresult = '';
	$sql_olt = $db->SimpleWhile("SELECT id, place, model, inf FROM switch WHERE device = 'olt'");
	...
	*/
	...
	if (isset($selectswitch) && $selectswitch > 0) {
		$tplresult .= "<a class='menu-sub' href='/?do=onuoffline{$url}'><img src='../style/img/pmon_return.png'>View all</a>"; 
	}
	foreach($array_switch as $oltid => $device){
		if(isset($device['swid']) && $access->get('dev' . $oltid)) {
			$count_ont = count($sqlonus[$oltid]['ont']);
		$tplresult .= "<a class='menu-sub' href='/?do=onuoffline{$url}&id=".$oltid."'>
			".$device['place']."<span class=\"badont\">{$count_ont}</span>		
			</a>";
		}
	}
	...
	if(isset($sqlonus) && count($sqlonus) > 0){
		...
		foreach($sqlonus as $oltid => $olt){
			$tplresult .= '<td colspan="10" class="reason_td"> 
					...
					<a href="/?do=onulist'.$url.'&id='.$masiv_switch[$oltid]['swid'].'">'.(isset($masiv_switch[$oltid]['place'])?$masiv_switch[$oltid]['place']:'').'</a>
			...
			foreach($olt['ont'] as $ontid => $onu){
				...
				$tplresult .= '<tr>
				<td class="status" '.(isset($ont['reason']) ? 'id="'.$ont['reason'].'"' : '').'>'.$get_status.'</td>
				<td class="td_url" style="text-align: left;">
				...
				<td>
					<span class="offs_">'.$onu['offline'].'</span>
				</td>				
				<td>
					<font color="#222"><b>'.aftertime($onu['offline']).'</b></font>
				</td>				
				<td>
					'.(!empty($onu['rx'])?signalTerminal($onu['rx']):'').'
				</td>
				';
				$tplresult .= '</tr>';
			}
