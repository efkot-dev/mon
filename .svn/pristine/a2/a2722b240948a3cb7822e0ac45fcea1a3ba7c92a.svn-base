...
$table .= '<table class="table_diagnostic"><tr><td>';
	...
	if (isset($list_ont) && count($list_ont) > 0) {
		...
		<th>Sn_Mac</th><th>Inface</th><th>Rx</th><th>Dist</th><th>Last Rx</th></tr></thead><tbody>';
		foreach ($list_ont as $onu) {
			...
			$table .= '<td>'.(!empty($onu['rx'])?signalTerminal($onu['rx']):'N/A').'</td>';
...
$table .= '</td><td>';
	// BAD SIGNAL
	$sql = "SELECT o.idonu, o.mac, o.sn, o.olt, o.portolt, o.inface, COUNT(h.id) AS bad_signal_count FROM onus o LEFT JOIN historysignal h ON o.idonu = h.onu AND h.signal < -24 AND h.datetime >= '{$last_month_date}' WHERE o.olt = '{$pon['oltid']}' AND o.portolt = '{$pon['sfpid']}' GROUP BY o.idonu HAVING bad_signal_count > 0 ORDER BY bad_signal_count DESC";
...
<div id="onu-speedbar">
	...
	<span class="brmspan"><i class="fi fi-rr-angle-left"></i>PON Signal Diagnostics</span>
</div>';
