...
class CDATA_1608sn { 
    ...
    private $configapionugponget = 'adminstatus,operstatus,dist,reason,regtime,temp,rx,tx,model,vendor,eth,name';
    private $filter = true;
    private $filters = [
	...
	public function Load(){
		...
		if($listonu){
			...
			if(is_array($indexonu)){
				...
				if(is_array($result)){
					$this->db->SQLupdate($PMonTables['onus'],['cron' => 2],['olt' => $this->id]);
				}
			}
		}
		if(is_array($result)){
	...
	public function savePort($dataPort){
		...
		if(!empty($dataPort['pon'])){
			...
			$this->db->SQLupdate($PMonTables['switch'],['updates_port' => $this->now],['id' => $this->id]);
		}
	}
    protected function savePortSwitch($data) {
		global $db, $PMonTables;
		if(!empty($data['llid'])){	
			$row = $this->db->Fast($PMonTables['switchport'],'*',['deviceid' => $this->id, 'llid' => $data['llid']]);
			if(!empty($row['id'])){
				
			}else{
				$this->db->SQLinsert($PMonTables['switchport'],['deviceid' => $this->id,'llid' => $data['llid'],'nameport' => $data['name'],'typeport' => $data['typeport'],'operstatus' => 'none','added' => $this->now]);
			}
		}
	} 
    ...
    protected function savePonSwitch($dataPort) {
		global $db, $PMonTables;
		$row = $this->db->Multi($PMonTables['switchpon'],'*',['oltid' => $this->id,'sfpid' => $dataPort['sfpid']]);
		if(!count($row)){
			$this->db->SQLinsert($PMonTables['switchpon'],['idportolt' => $dataPort['sfpid'],'support' => $dataPort['cardcount'],'sort' => $dataPort['sort'],'oltid' => $this->id,'pon' => $dataPort['name'],'sfpid' => $dataPort['sfpid'],'added' => $this->now]);
		}
		...
		$allonu = $this->db->Multi($PMonTables['onus'],'*',['olt' => $this->id,'portolt' => $dataPort['sfpid']]);
		if(!empty($dataPort['sfpid']))
			$this->db->SQLupdate($PMonTables['switchpon'],['count' =>count($allonu)],['sfpid' =>$dataPort['sfpid'],'oltid' => $this->id]);
	}
	public function tempSaveSignalSaveOnuGpon($dataOnu){	
		global $db, $PMonTables, $config;
		$savehistor = false;
		$onu = $this->db->Fast($PMonTables['onus'],'status,rx,idonu,inface,mac,sn,changerx,olt',['olt' => $this->id,'keyonu' => $dataOnu['keyonu']]);
		if(!empty($onu['idonu'])){
			$rx = $rx ?? null;
			if(!empty($dataOnu['rx'])){
				$rx = $this->clear_rx($dataOnu['rx']);
				if($rx){
					$this->db->SQLupdate($PMonTables['onus'],['rx' => $rx,'rating' => 1],['idonu' => $onu['idonu']]);
				}
			}
			...
			if(!empty($config['onugraph']) && $config['onugraph']=='on' && $savehistor && $rx){
				$this->db->SQLInsert($PMonTables['historyrx'],['device' => $this->id,'onu' => $onu['idonu'],'signal' => $rx,'datetime' => $this->now]);
			}
		}
	}
	...
	public function tempSaveOnuGpon($dataOnu = array()){	
		...
		if(!empty($dataOnu['keyonu'])){
			$arr = $this->db->Fast($PMonTables['onus'],'*',['keyonu' =>$dataOnu['keyonu'],'olt' => $dataOnu['id']]); 
			if(!empty($arr['idonu'])){
				...
				$this->db->SQLupdate($PMonTables['onus'],$SQLset,['idonu' => $arr['idonu']]);
			}else{				
				...
				$this->db->SQLinsert($PMonTables['onus'],$SQLinsert);
			}
		}
	}
	...
	public function getListOnuOnline(){	
		global $db, $PMonTables;
		$sqlList = $this->db->Multi($PMonTables['onus'],'keyonu,portolt,type',['olt' => $this->id, 'status' => 1]);
		if(is_array($sqlList)){
