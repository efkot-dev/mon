...
class CDATA_1616sn { 
    ...
    private $configapionugpon = 'dist,status,name,model,vendor';
    private $configapionugponget = 'dist,status,name,model,rx,vendor';
			...
			if(is_array($indexOnu)){
				foreach($indexOnu as $io => $eachsig) {
					...
					if(isset($line[0]) && isset($line[1])) {
						$result[$io] = array(
							'inface'=>$this->inface1616(trim($line[0])),
							'sn'=>SnHuawei($eachsig),'do' => 'onu',
							'id'=>$this->id,'pon'=>'gpon',
							'types'=>$this->primary,'keyonu'=> trim($line[0])
						);
					}	
				}
				if(is_array($result)){
					$this->db->SQLupdate('onus',['cron' => 2],['olt' => $this->id]);
				}
			}
		}
		if(is_array($result)){
