...
class SmartFiber_Epon{ 
    ...
    private $indexdevice;
    private $now;
    private $primary = 'status,name';
    private $configapionuepon = 'rx,operstatus,model,eth,vendor,temp,status,dist,name,volt,tx,vlan'; // regtime
    private $configapionueponget = 'rx,operstatus,model,eth,vendor,temp,status,dist,name,volt,tx,vlan'; // regtime
	...
	public function Load(){
		if(is_numeric($this->id)){		
			$listinface = '1.3.6.1.4.1.8888.1.13.3.1.1.9.0';
	...
	public function getnameportgcom($str){
		$str = str_replace('epon0/2', 'EPON 0',$str);	
		...
		if(is_array($indexport)){
			...
			foreach ($indexport as $idport => $valueport) {				
				...
				if(!empty($infport[1]) && !empty($infport[0])){
					...
					if(preg_match('/epon/i',$dataindexport) || preg_match('/e0/i',$dataindexport)){
    ...
    protected function savePonSwitch($dataport) {
		...
		if(!empty($row['id'])){
			...
			$allonu = $this->db->Multi($PMonTables['onus'],'*',['olt' => $this->id,'portolt' => $dataport['sort']]);
	...
	public function tempSaveSignalSaveOnuEpon($dataOnu){	
		...
		$onu = $this->db->Fast($PMonTables['onus'],'status,rx,idonu,inface,mac,sn,changerx,olt',['olt' => $this->id,'zte_idport' => $dataOnu['keyport'],'keyonu' => $dataOnu['keyonu']]);
		if(!empty($onu['idonu'])){
			$rx = $rx ?? null;
			if(!empty($dataOnu['rx'])){
	...
	public function tempSaveOnuEpon($dataOnu){	
		...
		if(!empty($dataOnu['keyonu'])){
			$arr = $this->db->Fast($PMonTables['onus'],'*',['keyonu' =>$dataOnu['keyonu'],'zte_idport' =>$dataOnu['keyport'],'olt' => $dataOnu['id']]); 
			if(!empty($arr['idonu'])){
	...
	public function getListOnuOnline(){	
		global $db, $PMonTables;
		$sqlList = $this->db->Multi($PMonTables['onus'],'keyonu,portolt,idonu,type,zte_idport',['olt' => $this->id, 'status' => 1]);
		$array = array();
		if(is_array($sqlList)){
