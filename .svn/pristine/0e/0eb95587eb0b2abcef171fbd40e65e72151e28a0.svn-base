...
class CDATA_FD1700sv3{ 
	...
	public function Port(){
			...
			foreach ($portlist as $io => $eachsig) {
				if (isset($eachsig) && !empty($eachsig['result'])) {
					...
					if(!preg_match('/:/i',$getinf)) {
						$nameport = $this->ClearNamePort($getinf);
						$listport[$io] = array(
							'llid' =>trim($io),
							'id' =>trim($io),
		...
		if(is_array($data['port'])){
			foreach($data['port'] as $llid => $valuePon){
				if(preg_match('/pon/i',$valuePon['name'])) {
					preg_match('/0\/(\d+)\/(\d+)/',$valuePon['name'],$mat);
					$support = '128';
					$listPon[$llid] = [
						'name' => $valuePon['name'],
						'sort' => $mat[2],
	...
	public function ClearNamePort($result){
	...
	public function Load(){
		$oid = [
			'oid' => '1.3.6.1.4.1.17409.2.8.4.1.1.3',
			'type' => 'class','deloid' => true,'ip' => $this->ip,'community'=> $this->community
		];
		...
		if(is_array($sqlList)){
			foreach($sqlList as $key => $value){
				if(!empty($value['keyonu']) && !empty($value['type']))
					$array[$key] = array('id'=>$this->id,'keyport' => $value['portolt'],'keyonu'=>$value['keyonu'],'pon'=>$value['type'],'do'=>'onu','types'=>'rx');
			}
		}
		return $array;
	}
	...
	public function convert_inface($data) {
		$shelf = floor(($data / 65536) / 215);
		$port = floor(($data % (256 * 256)) / 256) / 16;
		$numonu = $data % 128;
		return '0/' . $shelf . '/' . ($port + 1) . ':' . $numonu;
	}
