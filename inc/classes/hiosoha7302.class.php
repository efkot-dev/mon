...
class HiosoHA7302 { 
    ...
    private $configapionugpon = 'name,status,mac,tx,rx';
    private $filter = true;
	...
	public function Load(){
		$port = [
			'oid' => '1.3.6.1.4.1.25355.3.2.6.3.2.1.11.1',
			'type' => 'class','deloid' => false,'ip' => $this->ip,'community'=> $this->community
		];
		$result_epon = [];
		...
		if(is_array_empty($onulist)){
			foreach($onulist as $inface => $eachsig) {
				if(isset($eachsig['result'])) {
					preg_match('/(\d+).(\d+)/i',$inface,$datamatch);
					$mac = $this->clearData($eachsig['result']);
					$result_epon[$init_epon] = [
						'do' => 'onu','id'=>$this->id,'dist'=>100,'pon'=>'epon','mac'=>preg_replace('/(.{2})/','\1:',mb_strtolower($mac),5),					
						'inface'=>'0/'.$datamatch[1].':'.$datamatch[2],'checker'=>md5('0/'.$datamatch[1].':'.$datamatch[2]),
						'types'=>$this->primary,'keyonu'=> trim($datamatch[2]),'keyport'=> trim($datamatch[1])
					];
					$init_epon ++;
				}
			}
	...
	public function tempSaveSignalSaveOnuEpon($dataOnu){	
		...
		$onu = $this->db->Fast('onus','status,rx,idonu,inface,mac,sn,changerx,olt',['olt' => $this->id,'keyonu' => $dataOnu['keyonu'],'portolt' => $dataOnu['keyport']]);
		if(!empty($onu['idonu'])){
			$rx = $rx ?? null;
			if(!empty($dataOnu['rx'])){
	...
	public function tempSaveOnuEpon($dataOnu = array()){	
		...
		if(!empty($dataOnu['keyonu']) && !empty($dataOnu['keyport'])){
			...
			if(!empty($arr['idonu'])){
				...
				$sqlset = array('olt' => $dataOnu['id'],'updates' => $this->now,'added' => $this->now, ($statusONU==1?'online':'offline') => $this->now,'keyonu' => $dataOnu['keyonu'],'status' => $statusONU,'mac' => $dataOnu['mac'],'inface' => $dataOnu['inface'],'dist' => (!empty($dataOnu['dist']) ? $dataOnu['dist'] : 0),'name' => (!empty($dataOnu['name']) ? $dataOnu['name'] : ''),'reason' => (!empty($dataOnu['reason']) ? $dataOnu['reason'] : ''),'type' => $dataOnu['pon'],'cron' => 1,'zte_idport' => $dataOnu['keyport'],'portolt' => $dataOnu['keyport']);
