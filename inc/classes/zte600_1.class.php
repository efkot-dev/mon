...
class ZTE_c600_1 { 
	...
	private $now;
    private $configapionugpon = 'dist,status,reason,name,note,eth,model,vendor,rx,uptime,config';
	...
	protected function savePonPortSwitch($data){
		if(!empty($data['llid'])){	
			$row = $this->db->Simple("SELECT * FROM switch_port WHERE deviceid = '{$this->id}' AND llid = '{$data['llid']}' LIMIT 1");
			if(!$row['id']){
	...
	public function ScanerPort(){
		$startIndex = 285278465;
		$endIndex   = 285279000;
		$temp = [];
		$oid_base = '1.3.6.1.2.1.31.1.1.1.1'; // ifName OID
		for ($i = $startIndex; $i <= $endIndex; $i++) {
			$oid = "$oid_base.$i";
			$result = @snmp2_get($this->ip, $this->community, $oid);
			if ($result !== false) {
				$temp[$i]['result'] = $result;
			}
		}
		return $temp;
	}
		...
		if(is_array_empty($portlist)){
			foreach($portlist as $idport => $valueport['result']) {				
				if(!empty($valueport['result'])){
					...
					$name_real_port = $this->rep('",STRING: ,= ,olt-','',$valueport['result']);
					$listoltport[$idport] = [
    ...
    protected function savePonSwitch($dataPort) {
		...
		if ($row['count_this'] == 0) {
			...
			$name_pon = $this->rep(',olt- ','',$dataPort['name']);
			$this->db->SQLupdate('switch_pon',['pon' => $name_pon],['oltid' => $this->id, 'sfpid' => $dataPort['sfpid']]);
		}
		if(!empty($dataPort['sfpid']) && !empty($dataPort['sort'])){		
			$inf = $this->rep('GPON ,XPON ,EPON ,olt- ','',$dataPort['name']);
	...
	public function tempSaveOnuGpon($dataOnu = array()){	
		...
		if(!empty($dataOnu['keyonu']) && !empty($dataOnu['keyport'])){
			...
			if (!empty($temp['idonu'])) {
				$now = $this->now;
				$dist = $dataOnu['dist'] ?? $temp['dist'] ?? 1;
				$reason = !empty($dataOnu['reason']) ? $this->reason($dataOnu['reason']) : '';
				$zte_idport = $temp['zte_idport'] ?? $dataOnu['keyport'];
				$portolt = $temp['portolt'] ?? $dataOnu['keyport'];
				$newData = [
					'sw_shelf'   => $mat[1],
					'sw_slot'    => $mat[2],
					'sw_port'    => $mat[3],
				...
				if (isset($sqlset['status'])) {
					$sqlset[($onustatus == 1 ? 'online' : 'offline')] = $now;
				}
				if (!empty($sqlset)) {
					$sqlset['updates'] = $now;
					$this->db->SQLupdate('onus', $sqlset, ['idonu' => $temp['idonu']]);
				}
			}
			else{				
				$sqlinsert = [
