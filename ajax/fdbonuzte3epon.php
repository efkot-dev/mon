	...
	if(isset($get_onu['idonu']) && !empty($get_onu['idonu'])){
		...
		if(!empty($get_olt['netip'])){
			...
			$mac_epon = [
				...
				'type' => 'real','ip' => $get_olt['netip'],'community'=> $get_olt['snmpro']
			];
			...
			$reg = 'iso.3.6.1.4.1.3902.1082.40.10.2.1.2.1.50.1.'.$inface.'.';
			if(isset($indexmac) && $found){
				echo'<table class="resp-tab"><thead><tr><th width="15%">vlan</th><th width="25%">mac</th><th>type</th></tr></thead><tbody>';
				$i = 1;
				foreach($indexmac as $pi1s => $type) {
					$value = substr($pi1s, strlen($reg)); 
					$numbers = explode(".", $value);
					$first_vlan_number = $numbers[0];
					if(isset($first_vlan_number) && $first_vlan_number>0){
						$value_mac = substr($value, strlen($first_vlan_number) + 1);
