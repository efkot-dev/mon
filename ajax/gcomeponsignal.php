...
$sfp_tx_signal = '1.3.6.1.4.1.13464.1.13.4.1.1.6.0.'.$getONT['zte_idport'].'.'.$getONT['keyonu'];
$onu_tx_signal = '1.3.6.1.4.1.13464.1.13.3.3.1.7.0.'.$getONT['zte_idport'].'.'.$getONT['keyonu'];
$onu_rx_signal = '1.3.6.1.4.1.13464.1.13.3.3.1.8.0.'.$getONT['zte_idport'].'.'.$getONT['keyonu'];
$onu_volt = '1.3.6.1.4.1.13464.1.13.3.3.1.5.0.'.$getONT['zte_idport'].'.'.$getONT['keyonu'];
if(!empty($getOLT['netip']) && !empty($getOLT['class']) && $getOLT['oidid']==9){
	...
	if($onu_rx_value) {
		...
		echo'<div class="dbm_block"><div class="i"><img src="../style/img/rx.png"></div><div class="text"><div class="n">RX OMNU</div><div class="s"><span style="color:' . $olt_color . ';">' . $onu_rx_value . '</span><b>dBm</b></div></div></div>';
	}		
	...
	if($onu_tx_value) {
		...
		$minbad = (!empty($config['badsignalstart'])?$config['badsignalstart']:2);
		$olt_color = $onu_tx_value < 1.5 ? "red" : "#36b105";
		echo'<div class="dbm_block"><div class="i"><img src="../style/img/rx.png"></div><div class="text"><div class="n">TX ONU</div><div class="s"><span style="color:' . $olt_color . ';">' . $onu_tx_value . '</span><b>dBm</b></div></div></div>';
	}		
	...
	if($onu_volt_value) {
		...
		echo'<div class="dbm_block"><div class="i"><img src="../style/img/voltage.png"></div><div class="text"><div class="n">Onu Tx</div><div class="s"><span style="color:' . $olt_color . ';">' . $onu_volt_value . '</span><b>V</b></div></div></div>';
	}	
	echo'</div>';	
}
}
