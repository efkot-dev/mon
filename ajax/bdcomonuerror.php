...
if (!empty($sqlonu)) {
    foreach ($sqlonu as $onu) {
		$temponu[$onu['portolt']][$onu['keyonu']] = [
			'dist' => $onu['dist'],
			'status' => $onu['status'],
			'idonu' => $onu['idonu'],
			'keyonu' => $onu['keyonu'],
			'name' => $onu['name'],
			'sn' => (isset($onu['sn'])?$onu['sn']:$onu['mac']),
			'rx' => $onu['rx'],
			'lasterr' => $onu['lasterr'],
			'err' => $onu['err'],
			'inface' => $onu['inface']
...
echo '<table class="resp-tab"><thead><tr><th width="2%">status</th><th width="10%">interface</th><th width="10%">error</th><th width="5%">rx</th><th width="5%">dist</th><th>information</th></tr></thead><tbody>';
foreach ($arrayonus as $inface) {    
    if (!empty($inface['onu'])) {
        echo '<tr><td class="td_name" colspan="6">'.$inface['pon'].'</td></tr>';
        foreach ($inface['onu'] as $keyonu => $onus) {   
			if(!empty($onus['lasterr']) && $onus['lasterr']>0){		
				echo '<tr>';    
