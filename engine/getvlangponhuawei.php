...
if(!empty($switch['id'])){
	$sqlonu = $db->SimpleWhile("SELECT idonu, zte_idport, keyonu, olt, inface FROM onus WHERE status = '1' AND olt = '{$switch['id']}'");
	$pauseInterval = 40;
	$counter = 0;
	$vlan_array = array();
	if(isset($sqlonu) && count($sqlonu) > 0){
		foreach ($sqlonu as $onu){
			$onu_vlan = @snmp2_get($switch['netip'], $switch['snmpro'], '1.3.6.1.4.1.2011.6.128.1.1.2.62.1.7.' . $onu['zte_idport'].'.' . $onu['keyonu'].'.1', 100000, 5);
			if(isset($onu_vlan)){
