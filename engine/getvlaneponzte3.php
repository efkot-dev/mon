...
if(!empty($switch['id'])){
	$sqlonu = $db->SimpleWhile("SELECT idonu, zte_idport, keyonu, olt, inface FROM onus WHERE status = '1' AND type = 'epon' AND olt = '{$switch['id']}'");
	$pauseInterval = 40;
	$counter = 0;
	$vlan_array = array();
	if(isset($sqlonu) && count($sqlonu) > 0){
		foreach ($sqlonu as $onu){
			$onu_vlan = @snmp2_get($switch['netip'], $switch['snmpro'], '1.3.6.1.4.1.3902.1015.1010.1.1.1.10.2.1.1.' . $onu['keyonu'].'.1', 100000, 5);
			if(isset($onu_vlan)){
	...
	$logger->init(['log'=>'device','type'=>'monitor','descr'=>'Get EPON VLAN ','deviceid'=>$id_device,'who'=>'cron']);
